...
function kadabra_centrality(
    ...
    res = [global_approx[v] / phase2_n for v = 1:n]
    lower_bounds = Float64[max(0.0, res[v] - compute_f(res[v], phase2_n, delta_l_guess[v], omega)) for v = 1:n]
    upper_bounds = Float64[min(1.0, res[v] + compute_g(res[v], phase2_n, delta_u_guess[v], omega)) for v = 1:n]
