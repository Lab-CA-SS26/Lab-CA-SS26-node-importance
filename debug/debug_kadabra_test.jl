    ...
    if parallel
        ...
        Threads.@threads for tid = 1:nthreads
            ...
            for _ = 1:tau_per_thread
                s = rand(t_rng, 1:n)
                t = rand(t_rng, 1:n)
                println("s==t loop s=", s, " t=", t); while s == t
                    t = rand(t_rng, 1:n)
                end
        ...
        println("Phase 1 finished! Starting Phase 2...")
        flush(stdout)
        ...
        # Interval strikt deckeln, damit der Lock oft genug geprüft wird
        check_interval = min(10000, max(100, tau ÷ 100)) 
        
        check_lock = Threads.SpinLock()
        last_check = Threads.Atomic{Int}(tau_per_thread * nthreads)
        
        # HINWEIS: paths_sampled wurde komplett entfernt, da es UB/Data Races verursacht!
        ...
        Threads.@threads for tid = 1:nthreads
            ...
            while !stop_flag[] && n_pairs[] < omega
                ...
                for _ = 1:check_interval
                    GC.safepoint() # Muss INNERHALB der Schleife stehen!
                    s = rand(t_rng, 1:n)
                    t = rand(t_rng, 1:n)
                    println("s==t loop s=", s, " t=", t); while s == t
                        t = rand(t_rng, 1:n)
                ...
                Threads.atomic_add!(n_pairs, check_interval)

                if n_pairs[] % 1000 == 0
                    println("Thread $tid: n_pairs[] = $(n_pairs[]) / $omega")
                    flush(stdout)
                end
                local_pairs = 0
                ...
                if trylock(check_lock)
                    try
                        ...
                        if current_n_pairs_global - last_check[] >= check_interval * nthreads
                            ...
                            # Wir nutzen sicher den globalen, atomaren Wert
                            if check_finished(
                                global_approx,
                ...
                if tid == 1
                    println("Thread 1 finished while loop. stop_flag=$(stop_flag[]), n_pairs=$(n_pairs[])")
                    flush(stdout)
                end
            end
        end
...
function check_finished(
    ...
    for i = 1:n_tracked
        v = top_k_nodes[i]
        # min(1.0, ...) fängt asynchrone Auslesefehler ab und schützt vor DomainErrors
        bet[i] = min(1.0, approx_counts[v] / n_pairs) 
    end
...
function KadabraWorkspace(g::AbstractGraph{T}) where {T}
    ...
    visited_nodes = Vector{T}(undef, n + 2)
...
function _sample_shortest_path!(
    ...
    @inbounds begin
        println("BFS Loop cur_s=", cur_s_len, " cur_t=", cur_t_len); while !have_to_stop && (cur_s_len > 0 && cur_t_len > 0)
            GC.safepoint()
        ...
        if sp_edges_len > 0
            ...
            for i = 1:sp_edges_len
                ...
                if cur_weight >= rand_val || i == sp_edges_len
...
function _backtrack!(
    ...
    println("Backtrack Loop curr=", curr, " target=", target); @inbounds while curr != target
        GC.safepoint()
        counts[curr] += 1
