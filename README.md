...
Every measurement in the lab report is regenerated by a single script:

```bash
cd benchmark
./reproduce_report.sh                  # both stages
./reproduce_report.sh --stage bvk      # BRAVA-GNN vs KADABRA only (~20 min)
./reproduce_report.sh --stage tight    # C++ vs Julia at eps=1e-4 (hours)
```

It writes one JSON per run into `benchmark/results/reproduce/` and prints the LaTeX
table bodies (for `Report/tables/`) together with the summary statistics quoted in the
report text. Re-runs are incremental: existing JSON files are skipped, so delete a file
to recompute just that measurement.

The `tight` stage takes several hours (dominated by `amazon` and `dblp`), so launch it
under `tmux` on the compute server:

```bash
tmux new-session -d -s repro './reproduce_report.sh 2>&1 | tee ~/repro.log'
```

Note that `JULIA_NUM_THREADS` is what actually configures Julia's thread pool; the
runner's own `-t` flag is only recorded into the output JSON. The script sets the
environment variable for you and pins both implementations to the same thread count.

## 🚀 Running Individual Benchmarks on Server
